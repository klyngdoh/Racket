...
(define (ball-list-after-tick bl r)
  (cond
    [(= 1 (length bl)) (make-list 1 (change-ball (first bl) r))]
    [else (remove empty (append (make-list 1 (change-ball (first bl) r))
...
(define (add-ball-to-list w)
  (append (world-balls w) (make-list 1 (make-ball 330 384 3 -9))))
...
(define (set-all-ball-vel-zero bl)
  (cond
    [(empty? bl) empty]
    [else (append (make-list 1 (make-ball (ball-x (first bl))
                                          (ball-y (first bl)) 0 0))
...
(define (ball-position-list ballslist)
  (cond
    ...
    [else (append (make-list 1 (make-posn (ball-x (first ballslist))
                                          (ball-y (first ballslist))))
...
(define (world-to-scene w)
  (if (racket-selected? (world-racket w))
      (place-images (append (make-list (length (world-balls w)) BALL-IMAGE)
                            (make-list 1 RACKET-IMAGE)
                            (make-list 1 CURSOR-IMAGE))
                    (append (ball-position-list (world-balls w))
                            (make-list 1 (make-posn (get-racket-x w)
                                                    (get-racket-y w)))
                            (make-list 1 (make-posn (+ (get-racket-x w)
                                                       (get-racket-dx w))
                                                    (+ (get-racket-y w)
